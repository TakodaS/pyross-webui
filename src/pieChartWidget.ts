...
class pieChartWidget {
	...
	private _selectedAges: Set<string> | any
	private _pieChart: am4charts.PieChart | any
	private series: am4charts.PieSeries | any
	...
	private _miniChart: miniPieChart | any
	...
	//         this.series.slices.each(function (slice: any) {
	...
	public setAliasActiveOrInactive(num: any) {
		...
		if (num === 0) {
			//set all default
			holdClassThisContext.series.slices.each(function (slice: any) {
	...
	private initPieChart(): void {
		...
		this._pieChart.x = am4core.percent(0)
		this._pieChart.y = am4core.percent(-3)
		...
		this._pieChart.logo.dom.outerHTML = ""
	...
	private setEvents(): void {
		...
		this.slicesTemplate.events.on("hit", function (ev: any) {
			...
			if (!slice.isActive) {
				...
				holdClassThisContext.selectedAges.delete(
					ev.target.dataItem.dataContext["ageRange"]
				)
				slice.setState("aliasInactive")
				label.setState("aliasInactive")
			} else if (slice.isActive) {
				...
				holdClassThisContext.total = holdClassThisContext.total + hitValue
				holdClassThisContext.selectedAges.add(
					ev.target.dataItem.dataContext["ageRange"]
				)
				slice.setState("aliasActive")
				label.setState("aliasActive")
		...
		this.slicesTemplate.events.on("over", function (ev: any) {
		...
		this.slicesTemplate.events.on("out", function (ev: any) {
			let slice = ev.target
			let label = slice._dataItem._label
			if (slice.isActive) {
				slice.setState("aliasActive")
		...
		// this._pieChart.events.onAll(function (ev: any) {
		...
		this.series.slices.events.on("inserted", function (ev: any) {
			holdClassThisContext.countInsertedSlices++
			//console.log("INSERTED happened ", holdClassThisContext.countInsertedSlices, ev);
			//holdClassThisContext.checkIfAllSlicesAreSameColor();
			if (
				holdClassThisContext.countInsertedSlices ===
				holdClassThisContext.numberOfSlicesFromData
			) {
				holdClassThisContext.checkIfAllSlicesAreSameColor()
			}
		})

		///////////////////////// Pie Chart (slice: any) Label Events ////////////////
		//Propogate hit event on a pieseries label to the parent slice
		this.series.labels.template.events.on("hit", function (ev: any) {
			let parentSlice = ev.target._dataItem._slice
			parentSlice.dispatchImmediately("hit")
		})
		// this.series.labels.template.events.on("over", function (ev: any) {
		//     let parentSlice = ev.target._dataItem._slice;
		// });
		// this.series.labels.template.events.on("out", function (ev: any) {
		//     let label = ev.target;
		//     let parentSlice = ev.target._dataItem._slice;
		// });
	} //end setEvents
